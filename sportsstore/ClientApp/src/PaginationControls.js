...
export class PaginationControls extends Component {
    ...
    handlePageSizeChange = (ev) => { this.props.setPageSize(ev.target.value) }
    handleSortPropertyChange = (ev) => { this.props.setSortProperty(ev.target.value) }
        ...
        return <div className="m-2">
            ...
            <div className="form-inline justify-content-center">
                ...
                <select className="form-control" onChange={this.handleSortPropertyChange} value={this.props.sortKey || this.sortKeys[0]}>
                    {this.sortKeys.map(k => <option value={k.toLowerCase()} key={k}> Sort by {k}
                    </option>)}
                </select>
            </div>
        </div>
